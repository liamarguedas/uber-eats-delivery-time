def DateTime(data: pd.DataFrame):
	...
	data['OrderTime'] = pd.to_datetime(data['Time_Order_picked']).apply(lambda value: value.hour)
	data['TypeOfMeal'] = data['OrderTime'].apply(lambda value: "Breakfast" if value in [8, 9, 10, 11] else "Launch" if value in [12, 13, 14, 15, 16] else "Dinner")
	return data